function test_bridges_path(n)
    ...
    b = OPFGenerator.bridges(n, n-1, bus_fr, bus_to)
    @test all(values(b))
    ...
    append!(bus_fr, [n])
    append!(bus_to, [1])
    b = OPFGenerator.bridges(n, n, bus_fr, bus_to)
...
function test_bridge_star(n)
    ...
    b = OPFGenerator.bridges(n+1, n, bus_fr, bus_to)
    @test all(values(b))
    ...
    for i in 1:n
        bus_to[n+1] = i
        b = OPFGenerator.bridges(n+1, n+1, bus_fr, bus_to)
        @test !b[i]
        @test !b[n+1]
...
function test_bridge_edgecases()
    @test_throws ErrorException OPFGenerator.bridges(2, 1, [1], [1])
