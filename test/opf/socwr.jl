...
"""
    _test_socwr_parallelBranches()

Test dual feasibility of SOCWRConic problem with parallel branches.

This test is executed on the 118 bus system.
"""
function _test_socwr_parallelBranches()
    T = Float128
    ...
    # Identify buspairs
    buspairs = sort(unique((br["f_bus"], br["t_bus"]) for (_, br) in ref[:branch]))
    bp2idx = Dict((i, j) => k for (k, (i,j)) in enumerate(buspairs))
    IJ = length(buspairs)
    bp2br = [Int[] for _ in buspairs]
    ...
    ωf = [res["solution"]["branch"]["$(bp2br[k][1])"]["nu_voltage_prod_soc_1"] for k in 1:IJ]
    ωt = [res["solution"]["branch"]["$(bp2br[k][1])"]["nu_voltage_prod_soc_2"] for k in 1:IJ]
    ωr = [res["solution"]["branch"]["$(bp2br[k][1])"]["nu_voltage_prod_soc_3"] for k in 1:IJ]
    ωi = [res["solution"]["branch"]["$(bp2br[k][1])"]["nu_voltage_prod_soc_4"] for k in 1:IJ]
    ...
    for i in 1:N
        @test isapprox(
            ...
            + sum(
                ωf[k] / sqrt(2)
                for (k, (ik, jk)) in enumerate(buspairs) if ik == i;
                init=0.0
            )
            + sum(
                (g[e]+g_to[e]) * λpt[e] 
            ...
            + sum(
                ωt[k] / sqrt(2)
                for (k, (ik, jk)) in enumerate(buspairs) if jk == i;
                    init=0.0
            )
            + μ_w[i],
            0.0;
