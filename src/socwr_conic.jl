...
"""
function build_socwr_conic_opf(data::Dict{String,Any}, optimizer)
...
"""
function _extract_soc_conic_solution(model::JuMP.Model, data::Dict{String,Any})
    # Pre-process data
    ref = PM.build_ref(data)[:it][:pm][:nw][0]
    N = length(ref[:bus])
    G = length(ref[:gen])
    ...
    for b in 1:E
        if data["branch"]["$b"]["br_status"] == 0
            ...
            sol["branch"]["$b"] = Dict(
                ...
                "nu_voltage_prod_soc_1" => 0.0,
                "nu_voltage_prod_soc_2" => 0.0,
                "nu_voltage_prod_soc_3" => 0.0,
                "nu_voltage_prod_soc_4" => 0.0,
                "nu_sm_to_1" => 0.0,
                "nu_sm_to_2" => 0.0,
                "nu_sm_to_3" => 0.0,
                "nu_sm_fr_1" => 0.0,
            ...
            sol["branch"]["$b"] = Dict(
                ...
                # conic duals
                "nu_voltage_prod_soc_1" => dual(model[:voltage_prod_conic][bp])[1],
                "nu_voltage_prod_soc_2" => dual(model[:voltage_prod_conic][bp])[2],
                "nu_voltage_prod_soc_3" => dual(model[:voltage_prod_conic][bp])[3],
                "nu_voltage_prod_soc_4" => dual(model[:voltage_prod_conic][bp])[4],
                "nu_sm_to_1" => dual(model[:thermal_limit_to][b])[1],
                "nu_sm_to_2" => dual(model[:thermal_limit_to][b])[2],
                "nu_sm_to_3" => dual(model[:thermal_limit_to][b])[3],
                "nu_sm_fr_1" => dual(model[:thermal_limit_fr][b])[1],
                "nu_sm_fr_2" => dual(model[:thermal_limit_fr][b])[2],
                "nu_sm_fr_3" => dual(model[:thermal_limit_fr][b])[3],
            )
        end
    end 
